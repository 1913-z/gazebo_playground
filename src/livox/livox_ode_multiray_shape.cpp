                              ...
                              const ignition::math::Vector3d &_end)
{
    ...
    if (!this->collisionParent || !this->collisionParent->GetLink()) {
        gzerr << "Invalid collision parent or link\n";
        return;
    }
    
    physics::ODECollisionPtr odeCollision(
        new physics::ODECollision(this->collisionParent->GetLink()));
    
    if (!odeCollision) {
        gzerr << "Failed to create collision for ray\n";
        return;
    }
    ...
    physics::ODERayShapePtr ray(new physics::ODERayShape(odeCollision));
